...
export function addMapping(mapping: ReservationCartMapping) {
  ...
  const exists = mappings.find(
    (m) => m.reservationId === mapping.reservationId,
  );
  if (exists) {
    exists.cartItemIds = Array.from(
      new Set([...exists.cartItemIds, ...mapping.cartItemIds]),
    );
...
export function getMapping(
  reservationId: number,
): ReservationCartMapping | undefined {
  const mappings = readMappings();
  return mappings.find((m) => m.reservationId === reservationId);
}

export function getMappingByCartItemId(
  cartItemId: number,
): ReservationCartMapping | undefined {
  const mappings = readMappings();
