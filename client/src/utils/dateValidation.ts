...
export function resolveDateValidationMode({
    ...
    const normalizedContext = normalizeText([label, name, id].filter(Boolean).join(' '));
...
export function getDateValidationBounds(mode: Exclude<DateValidationMode, 'auto'> | 'none'): DateValidationBounds {
...
function getBoundsError(mode: Exclude<DateValidationMode, 'auto'> | 'none', reason: 'min' | 'max'): string {
    if (mode === 'birthDate') {
        return reason === 'max'
...
export function validateDateValue(rawDate: string, mode: DateValidationMode): string {
    if (!rawDate) return '';
